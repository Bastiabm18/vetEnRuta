...
export default function ConfirmationPage() {
  ...
  const sendConfirmationEmail = async (appointmentData: AppointmentData, idCita: string) => {
    // Validar datos esenciales antes de continuar
    ...
    try {
      // Inicializar EmailJS (importante hacerlo solo una vez)
      emailjs.init(publicKey);

      // Manejo de la fecha de Firestore Timestamp a Date para el email
      ...
      const templateParams = {
        // Usa 'owner_email' como el destinatario principal
        owner_email: appointmentData.datosDueño.email, 
        reply_to: 'veterinaria.concepcion2025@gmail.com', // Email de respuesta
        ...
        // NUEVO: Pasa el recargo por comuna al template de EmailJS
        comuna_fee: appointmentData.locationData?.costoAdicionalComuna?.toLocaleString('es-CL') || '0',

        total_appointment: calculateTotalAppointment().toLocaleString('es-CL'), // Usa la función actualizada
  ...
  useEffect(() => {
    const fetchAppointment = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Solo intenta enviar el email si aún no se ha enviado
          if (!emailSent) {
    ...
    fetchAppointment();
  }, [appointmentId, emailSent]); // emailSent en el array de dependencias para re-evaluar si se debe enviar

  // Asegura que la fecha se formatee correctamente para la visualización en la página
  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto text-black bg-white rounded-xl shadow-md overflow-hidden p-8">
        ...
        <div className="border-t border-b border-gray-200 py-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              {appointment.datosDueño?.estacionamiento && appointment.datosDueño.estacionamiento.trim() !== '' && ( // Solo muestra si hay texto no vacío
                <p className="mt-1 text-sm text-gray-900">
                  <span className="font-medium">Estacionamiento para el Veterinario:</span>{' '}
                  {appointment.datosDueño.estacionamiento} {/* Renderiza el string directamente */}
                </p>
              )}
            </div>
        ...
        </div> {/* Cierre del div principal de detalles de la cita */}

        {/* NUEVO: Mostrar el Recargo por Comuna si está presente y es mayor a 0 */}

...
// ✨ Importa las nuevas acciones e interfaces de citas-actions.ts
...
// ✨ INTERFACES PARA FILTROS (Importadas o definidas aquí para claridad si no se importan)
...
export default function ConsultasManager() {
  ...
  // ✨ ESTADOS PARA FILTROS DE REGION, COMUNA, y VETERINARIO
  ...
  // ✨ EFECTO PRINCIPAL para cargar citas cuando los filtros o el toggle cambian
  useEffect(() => {
    fetchCitas();
  }, [selectedRegionId, selectedComunaId, selectedVeterinarioId, showFinalized]); // ✨ Dependencias actualizadas

  // ✨ EFECTO para cargar Regiones y Veterinarios al montar
  useEffect(() => {
  ...
  // ✨ EFECTO para cargar Comunas cuando la Región seleccionada cambia
  useEffect(() => {
  ...
  const fetchCitas = async () => {
    setLoading(true);
    setError(null);
    // ✨ Pasa todos los filtros a getCitas
  ...
  const handleConfirmFinalizeCita = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const { totalAmount, ownerName, vetName } = result.data!;
  ...
  // Lógica de filtrado de citas a mostrar (ahora también por veterinario)
  ...
  return (
    <div className="container mx-auto p-4">
      <h2 className="text-3xl font-extrabold text-indigo-800 mb-8 text-center">Gestión General de Consultas</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredCitas.map((cita) => (
            <div key={cita.id} className="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-200 hover:shadow-xl transition-shadow duration-300">
              <div className="p-6">
                {/* ✨ Muestra el nombre del veterinario asignado */}
                    ...
                    <ul className="list-disc list-inside space-y-1">
                      {cita.mascotas.map((mascota, index) => (
                        <li key={index} className="text-gray-700 flex items-center flex-wrap">
                          ...
                          {mascota.nombre} ({mascota.tipo}, {mascota.sexo}, {mascota.edad})
                            ...
                            <div className="ml-6 mt-2 w-full border-l pl-4 border-gray-200">
                              ...
                              <ul className="list-disc list-inside text-sm space-y-1">
                                {mascota.servicios.map((servicio, svcIndex) => (
                                  <li key={svcIndex} className="text-gray-600 flex items-center">

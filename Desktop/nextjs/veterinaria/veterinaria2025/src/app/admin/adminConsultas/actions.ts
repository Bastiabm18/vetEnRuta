...
export async function getCitaById(citaId: string): Promise<{ cita?: Cita; error?: string }> {
  ...
  try {
    ...
    const calculatedTotalAmount = calculateTotalAmount({ mascotas: mappedMascotas, locationData: data.locationData as LocationData });
...
function calculateTotalAmount(citaData: { mascotas?: Mascota[], locationData?: LocationData }): number {
    let totalAmount = 0;

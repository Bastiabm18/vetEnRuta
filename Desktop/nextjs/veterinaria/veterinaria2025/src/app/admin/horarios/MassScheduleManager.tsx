// app/horarios/MassScheduleManager.tsx
"use client";
...
import {
  ...
  HoraDisponible,
  ComunaConValor // Importamos ComunaConValor (con valor: number)
} from './actions';
import { FaEdit, FaLock, FaUnlock, FaTimes } from 'react-icons/fa';
...
// Nueva interfaz que refleja el estado de la UI para valores de comuna
...
const MassScheduleManager = () => {
  ...
  // --- Efecto para cargar regiones y comunas al inicio ---
  useEffect(() => {
    const fetchData = async () => {
      const data = await getRegionesComunas();
      if (data?.regiones) {
  ...
  const handleComunaToggle = (comuna: Comuna) => {
    setSelectedComunasWithValues(prevSelected => {
      ...
      if (isSelected) {
        ...
        // Si no está seleccionada, la agregamos con su valor inicial vacío
  ...
  // Esta función ahora asume que todos los criterios están satisfechos cuando es llamada por useEffect
  const fetchSchedules = async () => {
    setLoadingSchedules(true);
    setMessage(null);

    try {
      // Las fechas están garantizadas como strings no vacíos aquí debido a la condición del useEffect
      ...
      const schedules = await getGeneratedSchedules(comunaIdsForFetch, start, end);
  ...
  // --- Efecto para recargar horarios al cambiar la selección de comunas o fechas ---
  useEffect(() => {
    // Solo intenta obtener los horarios si se cumplen todos los criterios necesarios
    if (selectedComunasWithValues.length > 0 && startDate && endDate) {
      fetchSchedules();
    } else {
      // Si los criterios no se cumplen, limpia los horarios y mensajes,
      // pero no muestra "No se encontraron horarios" inmediatamente.
      setGeneratedSchedules([]);
      setSelectedScheduleIds(new Set());
      setMessage(null);
      setLoadingSchedules(false); // Asegura que el estado de carga se reinicie si la obtención se omite
    }
  }, [selectedComunasWithValues, startDate, endDate]);
  ...
  const handleGenerateSchedule = async () => {
    ...
    // Validación de valores: verificar si hay valores inválidos o vacíos
    const hasInvalidValor = selectedComunasWithValues.some(sc => sc.valor === '' || isNaN(Number(sc.valor)) || Number(sc.valor) < 0);
    if (hasInvalidValor) {
      setMessage({ text: 'Por favor, ingresa un valor válido (número positivo) para cada comuna seleccionada.', type: 'error' });
      setLoadingGenerate(false);
      return;
    }

    try {
      // Mapear a ComunaConValor[] (valor: number) antes de enviar a la acción del servidor
      const comunasToSend: ComunaConValor[] = selectedComunasWithValues.map(sc => ({
        id: sc.id,
        nombre: sc.nombre,
        valor: Number(sc.valor) // Asegura que sea un número
      }));

      const result = await generateMassScheduleForUser(comunasToSend, startDate, endDate);
      if (result?.error) {
  ...
  const isScheduleExpired = (schedule: HoraDisponible): boolean => {
    const dateParts = schedule.fecha.split('-'); // ['YYYY', 'MM', 'DD']
    const timeParts = schedule.hora.split(':'); // ['HH', 'MM']
  ...
  const openEditComunaValuesModal = (schedule: HoraDisponible) => {
    ...
    // Mapear a ComunaValorUI[] para permitir '' en el valor del input
    setModalComunaValues(schedule.id_comuna.map(c => ({
      id: c.id,
      nombre: c.nombre,
      valor: c.valor === 0 ? '' : c.valor // Si el valor es 0, mostrar vacío en el input, sino el valor numérico
    })));
  ...
  const handleSaveComunaValues = async () => {
    ...
    // Validación de valores: verificar si hay valores inválidos o vacíos
    const hasInvalidValor = modalComunaValues.some(sc =>  isNaN(Number(sc.valor)) || Number(sc.valor) < 0);
    if (hasInvalidValor) {
      setModalError('Por favor, ingresa un valor válido (número positivo) para todas las comunas.');
      setModalLoading(false);
      return;
    }
    ...
    try {
      ...
      const comunasToSend: ComunaConValor[] = modalComunaValues.map(c => ({
        id: c.id,
        nombre: c.nombre,
        valor: Number(c.valor) // Asegura que sea un número antes de enviar
      }));
      ...
      if (result?.success) {
        ...
        fetchSchedules(); // Recargar horarios para ver los cambios
      } else if (result?.error) {
  ...
  return (
    <div className="bg-white text-gray-800 shadow rounded-lg p-6  font-inter">
      <h2 className="text-2xl font-bold text-indigo-700 mb-6 border-b pb-3">Gestión Horarios Mensuales</h2>

      <div className="mb-8 p-4 bg-gray-50 rounded-lg shadow-inner">
        <p className="text-base text-gray-700 mb-4">
          Selecciona una región, una o varias comunas y un rango de fechas para generar o visualizar los horarios disponibles.
        </p>
        ...
        <button onClick={handleGenerateSchedule} disabled={loadingGenerate || selectedComunasWithValues.length === 0 || !startDate || !endDate || selectedComunasWithValues.some(sc => sc.valor === '' || isNaN(Number(sc.valor)) || Number(sc.valor) < 0)}
          className={`px-6 py-2 rounded-lg text-white font-semibold text-lg shadow-md transition duration-200 ease-in-out transform hover:scale-105 ${
            loadingGenerate || selectedComunasWithValues.some(sc => sc.valor === '' || isNaN(Number(sc.valor)) || Number(sc.valor) < 0) ? 'bg-gray-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2'
          }`}>
          {loadingGenerate ? (
      ...
      <div className="mt-8">
        ...
        {/* Lógica de Renderizado Condicional Mejorada */}
        {loadingSchedules ? (
                  ...
                  return (
                    <tr key={schedule.id} className={expired ? 'bg-gray-50 opacity-80' : `hover:bg-indigo-50 transition duration-100 ${isChecked ? 'bg-indigo-100' : ''}`}>
                      ...
                      <td className={`px-4 py-3 whitespace-nowrap text-sm ${expired ? 'text-gray-500 line-through' : 'text-gray-900'}`}>
                        {(() => {
                          const parts = schedule.fecha.split('-'); // ['YYYY', 'MM', 'DD']
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm font-medium flex items-center space-x-2">
                        {/* Botón para abrir el modal de edición de valores de comuna */}
        ...
        ) : ( // Mensaje por defecto cuando no hay horarios generados o no se cumplen los criterios.
          <div className="bg-gray-50 p-6 rounded-lg text-center shadow-inner">
            <p className="text-gray-600 text-lg">No hay horarios disponibles para mostrar con los criterios seleccionados.</p>
          </div>
        )}
      </div>
